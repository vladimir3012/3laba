...
class Clock extends Product {
    haveMaxWeightIn: number;
    size: string;
    constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public maxWeightIn: number, size: string) {
        super(id, name, price, description, inStock);
        this.haveMaxWeightIn = maxWeightIn;
        this.size = size;
        this.Init();
    }
    ...
    let p = document.createElement("p");
    p.setAttribute("class", "card-text text-info m-0");
    p.innerHTML = "Есть по вместительости(кг): " + this.haveMaxWeightIn;
    obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
    }
        if(this.size!=null){
            let p = document.createElement("p");
...
function CheckSizeBig(flag: boolean) {

    document.getElementById('rowts').innerHTML = "";
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if ((<Clock>productList[i]).size == "большая")
                (<Clock>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
            productList[i].Init();
...
function CheckHaveMaxWeightIn10(flag: boolean) {

    document.getElementById('rowts').innerHTML = "";
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if ((<Clock>productList[i]).haveMaxWeightIn==10)
                (<Clock>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
...
class Plate extends Product {
    power: number;
    color: string;
    constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public pow: number, public colour: string) {
        super(id, name, price, description, inStock);
        this.power = pow;
        this.color = colour;
        this.Init();
    ...
    Init() {
        ...
        if(this.power!=null){
            let p = document.createElement("p");
            p.setAttribute("class", "card-text text-info m-0");
            p.innerHTML = "Мощность: " + this.power;
            obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
        }
        if(this.color!=null){
            let p = document.createElement("p");
...
let productList: Product[] = [
    ...
    new Clock(9, "Часы1", 10235, "Обычные часы.", 1, 10, "средние"),
    new Plate(10, "Тарелка", 1000, "Обычная тарелка.", 1, 160, "белый"),
    new Clock(11, "Часы2", 4342, "Обычные часы.", 1, 20, "маленькие"),
